...
# -*- coding: utf-8 -*-
# =============================================================================
# codes of GLM4, parametric modulation of subjective value
# =============================================================================
import os
import glob
import pandas as pd
...
def read_runID(subID):
    runID = []
    files = glob.glob('behavior_data/EV_contrast/GLM4/{}_LongDelay_*_decision_dm.tsv'.format(subID))
    for f in files:
...
def matrix_designer_dm(subID):
    ...
    for r in runID:
        ...
        decision_long = pd.read_csv('behavior_data/EV_contrast/GLM4/{}_LongDelay_fMRI_run-0{}_decision_dm.tsv'.format(subID, r), 
                                    sep='\t', header = None, names = colnames, index_col=False)
        decision_short = pd.read_csv('behavior_data/EV_contrast/GLM4/{}_ShortDelay_fMRI_run-0{}_decision_dm.tsv'.format(subID, r), 
                                    sep='\t', header = None, names = colnames, index_col=False)
        ...
        matrix_pm.to_csv('GLM4_results/{}/design_matrix_run-0{}.csv'.format(subID, r), sep=',', index=False)
        plot_design_matrix(matrix_pm, output_file = 'GLM4_results/{}/design_matrix_run-0{}.png'.format(subID, r))
...
def first_level_glm(sublist, smooth):
    for subID in sublist:
        ...
        if not os.path.exists('GLM4_results'):
            os.mkdir('GLM4_results')
        directory = 'GLM4_results/{}'.format(subID)
        ...
        # beta values
        decision_long_output['effect_size'].to_filename('GLM4_results/{}/decision_long_beta_{}.nii.gz'.format(subID, smooth_index))
        decision_short_output['effect_size'].to_filename('GLM4_results/{}/decision_short_beta_{}.nii.gz'.format(subID, smooth_index)) 
        decision_long_short_output['effect_size'].to_filename('GLM4_results/{}/decision_long_short_beta_{}.nii.gz'.format(subID, smooth_index))
        decision_short_long_output['effect_size'].to_filename('GLM4_results/{}/decision_short_long_beta_{}.nii.gz'.format(subID, smooth_index)) 
        
        # z values
        decision_long_output['z_score'].to_filename('GLM4_results/{}/decision_long_zstat_{}.nii.gz'.format(subID, smooth_index))
        decision_short_output['z_score'].to_filename('GLM4_results/{}/decision_short_zstat_{}.nii.gz'.format(subID, smooth_index)) 
        decision_long_short_output['z_score'].to_filename('GLM4_results/{}/decision_long_short_zstat_{}.nii.gz'.format(subID, smooth_index))
        decision_short_long_output['z_score'].to_filename('GLM4_results/{}/decision_short_long_zstat_{}.nii.gz'.format(subID, smooth_index)) 
...
def second_level_glm(sublist): 
    ...
    for subID in sublist:
        contrast_decision_long.append(load_img('GLM4_results/{}/decision_long_beta_smoothed.nii.gz'.format(subID)))
        contrast_decision_short.append(load_img('GLM4_results/{}/decision_short_beta_smoothed.nii.gz'.format(subID)))
        contrast_decision_long_short.append(load_img('GLM4_results/{}/decision_long_short_beta_smoothed.nii.gz'.format(subID)))
        contrast_decision_short_long.append(load_img('GLM4_results/{}/decision_short_long_beta_smoothed.nii.gz'.format(subID)))
    ...
    # saving model outputs
    if not os.path.exists('GLM4_results/group'):
        os.mkdir('GLM4_results/group')

    thresholded_decision_long.to_filename('GLM4_results/group/thresholded_decision_long_zscore.nii.gz')
    decision_long_cluster_table = get_clusters_table(thresholded_decision_long, threshold_decision_long, cluster_threshold=30)
    decision_long_cluster_table.to_csv('GLM4_results/thresholded_decision_long_cluster.csv')
    group_decision_long_output['z_score'].to_filename('GLM4_results/group/unthresholded_decision_long_zscore.nii.gz')
    ...
    group_decision_long_report.save_as_html('GLM4_results/group/group_decision_long.html')
    
    thresholded_decision_short.to_filename('GLM4_results/group/thresholded_decision_short_zscore.nii.gz')
    decision_short_cluster_table = get_clusters_table(thresholded_decision_short, threshold_decision_short, cluster_threshold=30)
    decision_short_cluster_table.to_csv('GLM4_results/thresholded_decision_short_cluster.csv')
    group_decision_short_output['z_score'].to_filename('GLM4_results/group/unthresholded_decision_short_zscore.nii.gz')
    group_decision_short_report = make_glm_report(model = group_decision_short, contrasts ='intercept', 
                                                  alpha=0.05, height_control="fdr", cluster_threshold=30)
    group_decision_short_report.save_as_html('GLM4_results/group/group_decision_short.html')
    
    thresholded_decision_long_short.to_filename('GLM4_results/group/thresholded_decision_long_short_zscore.nii.gz')
    ...
    decision_long_short_cluster_table.to_csv('GLM4_results/thresholded_decision_long_short_cluster.csv')
    group_decision_long_short_output['z_score'].to_filename('GLM4_results/group/unthresholded_decision_long_short_zscore.nii.gz')
    group_decision_long_short_report = make_glm_report(model = group_decision_long_short, contrasts ='intercept', 
                                                       alpha=0.05, height_control="fdr", cluster_threshold=30)
    group_decision_long_short_report.save_as_html('GLM4_results/group/group_decision_long_short.html')
    
    thresholded_decision_short_long.to_filename('GLM4_results/group/thresholded_decision_short_long_zscore.nii.gz')
    ...
    decision_short_long_cluster_table.to_csv('GLM4_results/thresholded_decision_short_long_cluster.csv')
    group_decision_short_long_output['z_score'].to_filename('GLM4_results/group/unthresholded_decision_short_long_zscore.nii.gz')
    group_decision_short_long_report = make_glm_report(model = group_decision_short_long, contrasts ='intercept', 
                                                       alpha=0.05, height_control="fdr", cluster_threshold=30)
    group_decision_short_long_report.save_as_html('GLM4_results/group/group_decision_short_long.html')
