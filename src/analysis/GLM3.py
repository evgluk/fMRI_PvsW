...
# -*- coding: utf-8 -*-
# =============================================================================
# codes of GLM3, parametric modulation of delay
# =============================================================================
import os
import glob
import pandas as pd
...
def read_runID(subID):
    runID = []
    files = glob.glob('behavior_data/EV_contrast/GLM3/{}_LongDelay_*_decision_dm.tsv'.format(subID))
    for f in files:
...
def matrix_designer_dm(subID):
    ...
    for r in runID:
        ...
        decision_long = pd.read_csv('behavior_data/EV_contrast/GLM3/{}_LongDelay_fMRI_run-0{}_decision_dm.tsv'.format(subID, r), 
                                    sep='\t', header = None, names = colnames, index_col=False)
        decision_short = pd.read_csv('behavior_data/EV_contrast/GLM3/{}_ShortDelay_fMRI_run-0{}_decision_dm.tsv'.format(subID, r), 
                                    sep='\t', header = None, names = colnames, index_col=False)
        ...
        matrix_pm.to_csv('GLM3_results/{}/design_matrix_run-0{}.csv'.format(subID, r), sep=',', index=False)
        plot_design_matrix(matrix_pm, output_file = 'GLM3_results/{}/design_matrix_run-0{}.png'.format(subID, r))
...
def first_level_glm(sublist, smooth):
    for subID in sublist:
        ...
        if not os.path.exists('GLM3_results'):
            os.mkdir('GLM3_results')
        directory = 'GLM3_results/{}'.format(subID)
        ...
        # beta values
        decision_long_output['effect_size'].to_filename('GLM3_results/{}/decision_long_pm_beta_{}.nii.gz'.format(subID, smooth_index))
        decision_short_output['effect_size'].to_filename('GLM3_results/{}/decision_short_pm_beta_{}.nii.gz'.format(subID, smooth_index)) 
        
        decision_long_short_output['effect_size'].to_filename('GLM3_results/{}/decision_long_short_pm_beta_{}.nii.gz'.format(subID, smooth_index))
        decision_short_long_output['effect_size'].to_filename('GLM3_results/{}/decision_short_long_pm_beta_{}.nii.gz'.format(subID, smooth_index)) 
        
        # z values
        decision_long_output['z_score'].to_filename('GLM3_results/{}/decision_long_pm_zstat_{}.nii.gz'.format(subID, smooth_index))
        decision_short_output['z_score'].to_filename('GLM3_results/{}/decision_short_pm_zstat_{}.nii.gz'.format(subID, smooth_index)) 
        decision_long_short_output['z_score'].to_filename('GLM3_results/{}/decision_long_short_pm_zstat_{}.nii.gz'.format(subID, smooth_index))
        decision_short_long_output['z_score'].to_filename('GLM3_results/{}/decision_short_long_pm_zstat_{}.nii.gz'.format(subID, smooth_index)) 
...
def second_level_glm(sublist): 
    ...
    for subID in sublist:
        contrast_decision_long.append(load_img('GLM3_results/{}/decision_long_pm_beta_smoothed.nii.gz'.format(subID)))
        contrast_decision_short.append(load_img('GLM3_results/{}/decision_short_pm_beta_smoothed.nii.gz'.format(subID)))
        contrast_decision_long_short.append(load_img('GLM3_results/{}/decision_long_short_pm_beta_smoothed.nii.gz'.format(subID)))
        contrast_decision_short_long.append(load_img('GLM3_results/{}/decision_short_long_pm_beta_smoothed.nii.gz'.format(subID)))
    ...
    # saving model outputs
    if not os.path.exists('GLM3_results/group'):
        os.mkdir('GLM3_results/group')

    thresholded_decision_long.to_filename('GLM3_results/group/thresholded_decision_long_pm_zscore.nii.gz')
    decision_long_cluster_table = get_clusters_table(thresholded_decision_long, threshold_decision_long, cluster_threshold=30)
    decision_long_cluster_table.to_csv('GLM3_results/thresholded_decision_long_pm_cluster.csv')
    group_decision_long_output['z_score'].to_filename('GLM3_results/group/unthresholded_decision_long_pm_zscore.nii.gz')
    ...
    group_decision_long_report.save_as_html('GLM3_results/group/group_decision_long_pm.html')
    
    thresholded_decision_short.to_filename('GLM3_results/group/thresholded_decision_short_pm_zscore.nii.gz')
    decision_short_cluster_table = get_clusters_table(thresholded_decision_short, threshold_decision_short, cluster_threshold=30)
    decision_short_cluster_table.to_csv('GLM3_results/thresholded_decision_short_pm_cluster.csv')
    group_decision_short_output['z_score'].to_filename('GLM3_results/group/unthresholded_decision_short_pm_zscore.nii.gz')
    group_decision_short_report = make_glm_report(model = group_decision_short, contrasts ='intercept', 
                                                  alpha=0.05, height_control="fdr", cluster_threshold=30)
    group_decision_short_report.save_as_html('GLM3_results/group/group_decision_short_pm.html')
    
    thresholded_decision_long_short.to_filename('GLM3_results/group/thresholded_decision_long_short_pm_zscore.nii.gz')
    ...
    decision_long_short_cluster_table.to_csv('GLM3_results/thresholded_decision_long_short_pm_cluster.csv')
    group_decision_long_short_output['z_score'].to_filename('GLM3_results/group/unthresholded_decision_long_short_pm_zscore.nii.gz')
    group_decision_long_short_report = make_glm_report(model = group_decision_long_short, contrasts ='intercept', 
                                                       alpha=0.05, height_control="fdr", cluster_threshold=30)
    group_decision_long_short_report.save_as_html('GLM3_results/group/group_decision_long_short_pm.html')
    
    thresholded_decision_short_long.to_filename('GLM3_results/group/thresholded_decision_short_long_pm_zscore.nii.gz')
    ...
    decision_short_long_cluster_table.to_csv('GLM3_results/thresholded_decision_short_long_pm_cluster.csv')
    group_decision_short_long_output['z_score'].to_filename('GLM3_results/group/unthresholded_decision_short_long_pm_zscore.nii.gz')
    group_decision_short_long_report = make_glm_report(model = group_decision_short_long, contrasts ='intercept', 
                                                       alpha=0.05, height_control="fdr", cluster_threshold=30)
    group_decision_short_long_report.save_as_html('GLM3_results/group/group_decision_short_long_pm.html')
